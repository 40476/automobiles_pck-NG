...
local function get_vehicle_pitch_roll(pos, wheels, yaw)
  ...
  -- Get terrain height for each wheel
  ...
  -- Calculate the coefficients of the plane Ax + By + Cz + D = 0
  ...
  -- Create two vectors from the four wheels
  local v1 = { x = x2 - x1, y = y2 - y1, z = z2 - z1 }
  local v2 = { x = x3 - x1, y = y3 - y1, z = z3 - z1 }
  local v3 = { x = x4 - x1, y = y4 - y1, z = z4 - z1 }

  -- Take the average of the cross products between the three vectors
  local normal1 = {
  ...
  -- Average of two normals for better accuracy
  local normal = {
  ...
  if magnitude == 0 then
    return 0, 0     -- Returns neutral angles if calculation fails
  end
  ...
  -- Create a vehicle direction vector (from rear to front)
  local forward = {
  ...
  -- Find out if the vehicle is pointing forward or backward on the Z axis
  local direction_z = forward.z >= 0 and -1 or 1
  -- Find out if the vehicle is pointing right/left on the X axis
  local direction_x = forward.x >= 0 and -1 or 1

  -- Check which axis is "pointing forward"
...
function automobiles_lib.ground_get_distances(self, radius, axis_distance)
  ...
  if math.abs(front_wheel_x) < 0.01 then front_wheel_x = 0.01 end
  local rear_wheel_x = self._rear_wheel_xpos / 10
  if math.abs(rear_wheel_x) < 0.01 then rear_wheel_x = 0.01 end

  local wheels = {
    { x = -front_wheel_x, y = 0, z = axis_distance },   -- Left front wheel
    { x = front_wheel_x,  y = 0, z = axis_distance },   -- Right front wheel
    { x = -rear_wheel_x,  y = 0, z = -0 },              -- Left rear wheel
    { x = rear_wheel_x,   y = 0, z = -0 }               -- Right rear wheel
  }
